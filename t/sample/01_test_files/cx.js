    ...
    cX.library = {
        ...
        getUserId: function(createIfMissing) {
            ...
            if (createIfMissing !== false) {
                ...
                if (!persistentUserIdCookie) {
                    try { this.setCookie('cX_P', persistentUserId, 365, '/'); } catch (e) { }
                }
                if (!persistentUserIdLocalStorage) {
        ...
        _sendMessageToChild: function(childFrame, childSrc, message, domain) {
            if (this.hasPostMessage()) {
                if (childFrame.contentWindow && childFrame.contentWindow.postMessage) {
                    // For modern browsers, use the native postMessage
                    if (location.hostname.indexOf('.cxense.com') > -1 || location.hostname.indexOf('.cxpublic.com') > -1 || this.m_scriptStartTime % 10 === 1) {
            ...
            if (isRenderTemplateRequest || args.isSlave) {
                ...
                if (!allArgs.adSpaceId && !allArgs.adSpaceIds && (allArgs.widgetId || requestObject.widgetId)) {
                    ...
                    if (allArgs.usi) {
                        ...
                        (user.ids || (user.ids = {})).usi = allArgs.usi;
                    }
                    if (allArgs.ctx) {
                        var context = requestObject.context || (requestObject.context = {});
                        if (!context.url) {
                            context.url = allArgs.ctx;
                        }
                    }
                            ...
                            if (this.m_widgetSpecs.hasOwnProperty(widgetElId)) {
                                ...
                                if (widgetSpec && widgetSpec.hasOwnProperty('element') && widgetSpec.element) {
                                    ...
                                    if (parentMetrics) {
                                        ...
                                        overlapLeft = Math.max(parentMetrics.overlapPos.left, overlapLeft);
                                        overlapRight = Math.min(parentMetrics.overlapPos.left + parentMetrics.overlapSize.width, overlapRight);
                                        overlapTop = Math.max(parentMetrics.overlapPos.top, overlapTop);
                                        overlapBottom = Math.min(parentMetrics.overlapPos.top + parentMetrics.overlapSize.height, overlapBottom);
                                    }
